...
def test_cnn():
    # todo 加载数据, 修改为你自己的数据集的路径
    train_ds, test_ds, class_names = data_load("./data/scidb_mineral_data/train",
                                              "./data/scidb_mineral_data/test", 299, 299, 4)
    # todo 加载模型，修改为你的模型名称
    # base_model = train.PowerTransferMode()
    # model = base_model.InceptionResNetV2_model(
    #    nb_classes=8,
    #    img_rows=299,
    #    img_cols=299,
    #    is_plot_model=False
    #)
    # 从检查点恢复权重
    # saved_weights = './ckpt/widgets_28-0.96.h5'
    ...
    # title, x_labels, y_labels, harvest
    show_heatmaps(title="heatmap", x_labels=class_names, y_labels=class_names, harvest=heat_maps_float,
                  save_name="results/heatmap_InceptionResNetV2.png")
